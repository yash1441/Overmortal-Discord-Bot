...
module.exports = {
	...
	async execute(interaction) {
		...
		const platformSelectMenu = new StringSelectMenuBuilder()
			...
			.setPlaceholder(
				localization.select_platform[interaction.locale] ??
					localization.select_platform["en-US"]
			)
			.addOptions(
		...
		await interaction.editReply({
			content: bold(
				localization.select_submission_platform[interaction.locale] ??
					localization.select_submission_platform["en-US"]
			),
			components: [row],
		});
		...
		collector.on("collect", async (platformInteraction) => {
			...
			const creatorModalChannel = new ActionRowBuilder().addComponents(
				new TextInputBuilder()
					...
					.setLabel(
						...
						).channel_link[interaction.locale] ??
							JSON.parse(
								JSON.stringify(localization).replaceAll(
									"{{platform}}",
									creatorData.platform
			...
			const creatorModalSubs = new ActionRowBuilder().addComponents(
				new TextInputBuilder()
					.setCustomId("subs")
					.setLabel(
						localization.channel_subscribers[interaction.locale] ??
							localization.channel_subscribers["en-US"]
					)
					.setStyle(TextInputStyle.Short)
			);
			const creatorModalIntro = new ActionRowBuilder().addComponents(
				new TextInputBuilder()
					.setCustomId("intro")
					.setLabel(
						localization.brief_introduction[interaction.locale] ??
							localization.brief_introduction["en-US"]
					)
					.setStyle(TextInputStyle.Paragraph)
			);

			creatorModal.addComponents(
				creatorModalChannel,
				creatorModalSubs,
				creatorModalIntro
			);
			...
			const modalReply = await platformInteraction
				.awaitModalSubmit({
					time: 60_000,
					filter: (modalInteraction) =>
						modalInteraction.user.id ===
						platformInteraction.user.id,
				})
				.catch(() => {
			...
			await modalReply.reply({
				content: bold(creatorData.platform),
				ephemeral: true,
			});
			...
			creatorData.channel =
				modalReply.fields.getTextInputValue("channel");
			creatorData.subs = modalReply.fields.getTextInputValue("subs");
			creatorData.intro = modalReply.fields.getTextInputValue("intro");

			await interaction.editReply({
				content:
					(localization.application_submitted[interaction.locale] ??
						localization.application_submitted["en-US"]) +
					"\nPlatform " +
					inlineCode(creatorData.platform) +
					"\nChannel " +
					inlineCode(creatorData.channel) +
					"\nSubscribers " +
					inlineCode(creatorData.subs),
				components: [],
			});
			...
			if (records.total)
				return console.log(creatorData.name, " already submitted.");
